...
import { leadershipRoles, leadershipStats, type LeadershipIconKey } from "@/content/portfolio";

const iconMap: Record<LeadershipIconKey, React.ReactNode> = {
  users: (
  ...
  return (
    <SectionWrapper id="leadership" title="Leadership & Community" index="05">
      ...
      >
        <div className="relative p-8 rounded-xl glass border border-accent/15 overflow-hidden group card-hover">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-6">
            <div className="text-accent shrink-0">{iconMap[lead.icon]}</div>
      ...
      <div className="grid md:grid-cols-3 gap-5 mb-12">
        {leadershipRoles.slice(1).map((role, i) => (
          <motion.div
            key={role.title}
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              <span className="text-accent">{iconMap[role.icon]}</span>
              <span className="font-mono text-xs text-muted">
                {role.period}
              </span>
            </div>
